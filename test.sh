...
echo "=========================================="
echo "  FOCUS Corporation - API Test"
echo "=========================================="
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" $API_URL/api/tags)
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer $API_TOKEN" $API_URL/api/tags)
...
MODELS=$(curl -s -H "Authorization: Bearer $API_TOKEN" $API_URL/api/tags | jq -r '.models[].name' 2>/dev/null)
if [ -n "$MODELS" ]; then
